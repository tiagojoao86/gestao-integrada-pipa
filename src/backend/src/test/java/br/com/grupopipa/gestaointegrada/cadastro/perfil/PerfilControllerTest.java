...
/**
 * Testes unitários para PerfilController.
...
@WebMvcTest(value = PerfilController.class,
...
class PerfilControllerTest {
    ...
    void deveListarPerfisPaginados() throws Exception {
        ...
        PageDTO<PerfilGridDTO> pageDTO = new PageDTO<>(
                List.of(gridDTO),
                PageRequest.of(0, 10),
                1L
        );
        ...
        // When & Then
        mockMvc.perform(post("/perfil/query")
                        .with(csrf())
    ...
    void deveCriarNovoPerfil() throws Exception {
        // Given
        PerfilDTO novoDTO = PerfilDTO.builder()
                .nome("Operador")
                .build();
    ...
    void deveBuscarPerfilPorId() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/perfil/find-by-id")
                        .param("id", perfilId.toString()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.statusCode").value(200))
                .andExpect(jsonPath("$.body.nome", is("Administrador")))
                .andExpect(jsonPath("$.body.permissoes", hasSize(1)));
    ...
    void deveDeletarPerfil() throws Exception {
        ...
        // When & Then
        mockMvc.perform(delete("/perfil/{id}", perfilId)
                        .with(csrf()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.statusCode").value(200))
