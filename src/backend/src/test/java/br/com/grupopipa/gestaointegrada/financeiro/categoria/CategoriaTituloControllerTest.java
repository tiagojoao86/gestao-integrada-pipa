...
@WebMvcTest(value = CategoriaTituloController.class, excludeFilters = @ComponentScan.Filter(type = FilterType.REGEX, pattern = ".*TenantFilter"))
@AutoConfigureMockMvc(addFilters = false)
class CategoriaTituloControllerTest {
    ...
    @MockitoBean
    private CategoriaTituloService service;

    private CategoriaTituloDTO dtoValido;
    private CategoriaTituloGridDTO gridDTO;
    ...
    void setup() {
        ...
        dtoValido = CategoriaTituloDTO.builder()
                .id(categoriaId)
                .nome("Categoria Teste")
                .descricao("Descrição")
                .build();

        gridDTO = CategoriaTituloGridDTO.builder()
                .id(categoriaId)
                .nome("Categoria Teste")
                .descricao("Descrição")
                .build();
    ...
    void deveListarCategoriasPaginadas() throws Exception {
        ...
        PageDTO<CategoriaTituloGridDTO> pageDTO = new PageDTO<>(List.of(gridDTO),
        ...
        mockMvc.perform(post("/categoria-titulo/query").with(csrf())
    ...
    void deveCriarNovaCategoria() throws Exception {
        when(service.save(any(CategoriaTituloDTO.class))).thenReturn(dtoValido);

        mockMvc.perform(post("/categoria-titulo").with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
        ...
        verify(service, times(1)).save(any(CategoriaTituloDTO.class));
    }
    ...
    void deveBuscarCategoriaPorId() throws Exception {
        ...
        mockMvc.perform(get("/categoria-titulo/find-by-id").param("id", categoriaId.toString())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.statusCode", is(200)))
                .andExpect(jsonPath("$.body.id", is(categoriaId.toString())))
    ...
    void deveDeletarCategoria() throws Exception {
        ...
        mockMvc.perform(delete("/categoria-titulo/{id}", categoriaId).with(csrf())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.statusCode", is(200)));
