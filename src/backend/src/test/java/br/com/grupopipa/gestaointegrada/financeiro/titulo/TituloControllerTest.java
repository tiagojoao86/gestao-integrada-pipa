...
/**
 * Testes unitários para TituloController - CRUD básico. Usa MockMvc para testar os endpoints REST
 * sem subir o servidor.
 */
@DisplayName("TituloController - Testes Unitários")
@WebMvcTest(
    value = TituloController.class,
...
class TituloControllerTest {
  ...
  @MockitoBean private TituloService service;
  ...
  void deveListarTitulosPaginados() throws Exception {
    ...
    PageDTO<TituloGridDTO> pageDTO = new PageDTO<>(List.of(gridDTO), PageRequest.of(0, 10), 1L);
  ...
  void deveDeletarTitulo() throws Exception {
    ...
    mockMvc
        .perform(
            delete("/titulo/{id}", tituloId).with(csrf()).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.statusCode", is(200)));
