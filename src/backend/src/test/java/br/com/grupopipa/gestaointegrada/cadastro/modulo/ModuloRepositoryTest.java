...
/**
 * Testes de integração para ModuloRepository.
 * Valida consultas de módulos do sistema (entidade somente leitura).
 */
...
class ModuloRepositoryTest extends AbstractIntegrationTest {
    ...
    void deveVerificarGruposModulosDistintos() {
        ...
        long gruposDistintos = todosModulos.stream()
            .map(ModuloEntity::getGrupo)
            .distinct()
            .count();
