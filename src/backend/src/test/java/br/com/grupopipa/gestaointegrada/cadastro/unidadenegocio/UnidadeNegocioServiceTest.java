...
/**
 * Testes unitários para UnidadeNegocioService.
...
class UnidadeNegocioServiceTest {
    ...
    void deveCriarNovaUnidadeNegocio() {
        // Given
        when(repository.save(any(UnidadeNegocio.class))).thenAnswer(invocation -> {
    ...
    void deveAtualizarUnidadeNegocioExistente() {
        ...
        when(repository.save(any(UnidadeNegocio.class))).thenAnswer(invocation -> invocation.getArgument(0));
