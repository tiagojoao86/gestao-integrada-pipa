...
/**
 * Testes de integração para PlanoContasRepository. Valida a persistência e consultas do plano de
 * contas.
 */
...
class PlanoContasRepositoryTest extends AbstractIntegrationTest {
  ...
  void deveSalvarERecuperarPlanoContasRaiz() {
    // Given
    String codigo =
        (UUID.randomUUID().toString() + System.nanoTime()).replace("-", "").substring(0, 18);
    PlanoContas planoReceitas =
  ...
  void deveValidarConstraintCodigoUnico() {
    // Given
    String uniqueCodigo =
        ("3" + UUID.randomUUID().toString() + System.nanoTime()).replace("-", "").substring(0, 18);
