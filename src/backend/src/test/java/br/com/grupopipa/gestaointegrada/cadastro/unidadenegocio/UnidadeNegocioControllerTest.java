...
/**
 * Testes unitários para UnidadeNegocioController. Usa MockMvc para testar os endpoints REST sem
 * subir o servidor. Usa @MockitoBean para simular o service. @AutoConfigureMockMvc(addFilters =
 * false) desabilita os filtros de segurança, permitindo testar apenas a lógica do controller
...
@WebMvcTest(
    value = UnidadeNegocioController.class,
...
class UnidadeNegocioControllerTest {
  ...
  @MockitoBean private UnidadeNegocioService service;
  ...
  void deveListarUnidadesDeNegocio() throws Exception {
    ...
    UnidadeNegocioGridDTO gridDTO =
        UnidadeNegocioGridDTO.builder()
            .id(id)
            .codigo("UN001")
            .nome("Unidade Teste")
            .ativa(true)
    ...
    PageDTO<UnidadeNegocioGridDTO> pageDTO =
  ...
  void deveBuscarUnidadeDeNegocioPorId() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(get("/unidade-negocio/find-by-id").param("id", id.toString()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.statusCode").value(200))
  ...
  void deveDeletarUnidadeDeNegocio() throws Exception {
    ...
    // When/Then
    mockMvc
        .perform(delete("/unidade-negocio/{id}", id).with(csrf()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.statusCode").value(200))
