...
CREATE TABLE plano_contas (
    ...
    CONSTRAINT chk_plano_contas_tipo CHECK (tipo IN ('RECEITA', 'DESPESA', 'ATIVO', 'PASSIVO'))
);

COMMENT ON TABLE plano_contas IS 'Plano de contas com estrutura hierárquica.';
...
CREATE TABLE conta_bancaria (
    ...
    CONSTRAINT chk_conta_tipo CHECK (tipo IN ('CORRENTE', 'POUPANCA', 'CAIXA', 'INVESTIMENTO'))
);

COMMENT ON TABLE conta_bancaria IS 'Contas bancárias e caixas da organização.';
...
VALUES 
    (gen_random_uuid(), 'CADASTRO_PLANO_CONTAS', 'Plano de Contas', 'FINANCEIRO'),
    (gen_random_uuid(), 'CADASTRO_CONTA_BANCARIA', 'Contas Bancárias', 'FINANCEIRO');
...
WHERE p.nome = 'Administrador Geral'
  AND m.chave IN ('CADASTRO_PLANO_CONTAS', 'CADASTRO_CONTA_BANCARIA');
