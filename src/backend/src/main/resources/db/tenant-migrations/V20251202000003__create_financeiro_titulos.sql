...
CREATE TABLE titulo (
    ...
    CONSTRAINT fk_titulo_plano_contas FOREIGN KEY (plano_contas_id) REFERENCES plano_contas(id),
    ...
    CONSTRAINT chk_titulo_tipo CHECK (tipo IN ('A_PAGAR', 'A_RECEBER')),
    CONSTRAINT chk_titulo_status CHECK (status IN ('ABERTO', 'PARCIAL', 'PAGO', 'CANCELADO', 'VENCIDO')),
    ...
    CONSTRAINT chk_titulo_datas CHECK (data_vencimento >= data_emissao),
    CONSTRAINT chk_titulo_parcelamento CHECK (
        (numero_parcela IS NULL AND total_parcelas IS NULL AND titulo_origem_id IS NULL) OR
        (numero_parcela IS NOT NULL AND total_parcelas IS NOT NULL AND numero_parcela <= total_parcelas)
    )
);

COMMENT ON TABLE titulo IS 'Títulos financeiros - regime de competência (compromissos a pagar/receber).';
...
CREATE INDEX idx_titulo_plano_contas ON titulo(plano_contas_id);
...
CREATE TABLE movimentacao_financeira (
    ...
    CONSTRAINT fk_movimentacao_titulo FOREIGN KEY (titulo_id) REFERENCES titulo(id) ON DELETE CASCADE,
    CONSTRAINT fk_movimentacao_conta FOREIGN KEY (conta_bancaria_id) REFERENCES conta_bancaria(id),
    CONSTRAINT chk_movimentacao_tipo CHECK (tipo IN ('PAGAMENTO', 'RECEBIMENTO', 'ESTORNO', 'TRANSFERENCIA')),
    CONSTRAINT chk_movimentacao_forma CHECK (forma_pagamento IN ('PIX', 'DINHEIRO', 'BOLETO', 'CARTAO_CREDITO', 'CARTAO_DEBITO', 'TED', 'DOC', 'CHEQUE', 'DEPOSITO')),
    CONSTRAINT chk_movimentacao_valor CHECK (valor > 0)
);

COMMENT ON TABLE movimentacao_financeira IS 'Movimentações financeiras - regime de caixa (dinheiro real nas contas).';

CREATE INDEX idx_movimentacao_data ON movimentacao_financeira(data);
CREATE INDEX idx_movimentacao_titulo ON movimentacao_financeira(titulo_id);
...
VALUES 
    (gen_random_uuid(), 'FINANCEIRO_TITULO', 'Títulos Financeiros', 'FINANCEIRO'),
    (gen_random_uuid(), 'FINANCEIRO_MOVIMENTACAO', 'Movimentações Financeiras', 'FINANCEIRO');
...
WHERE p.nome = 'Administrador Geral'
  AND m.chave IN ('FINANCEIRO_TITULO', 'FINANCEIRO_MOVIMENTACAO');
