...
/**
 * Entidade principal do domínio financeiro - representa títulos a pagar/receber (regime de competência)
 */
@Entity
...
public class Titulo extends BaseEntity {
    ...
    public Titulo(TipoTitulo tipo, String descricao, Pessoa pessoa, PlanoContas planoContas,
                  Money valorOriginal, LocalDate dataEmissao, LocalDate dataVencimento) {
        validarCamposObrigatorios(tipo, descricao, pessoa, planoContas, valorOriginal, dataEmissao, dataVencimento);
        this.tipo = tipo;
    ...
    private void validarCamposObrigatorios(TipoTitulo tipo, String descricao, Pessoa pessoa, 
                                          PlanoContas planoContas, Money valorOriginal,
        ...
        if (valorOriginal == null || valorOriginal.isZero() || valorOriginal.isNegative()) {
        ...
        if (dataVencimento == null) {
            ...
            violations.add(new BeanValidationMessage("dataVencimento", "Data de vencimento não pode ser anterior à data de emissão"));
        }
    ...
    public void aplicarDesconto(Money desconto) {
        ...
        if (desconto == null || desconto.isNegative()) {
            ...
            violations.add(new BeanValidationMessage("valorDesconto", "Desconto não pode ser maior que o valor original"));
        }
    ...
    public void definirParcelamento(Integer numeroParcela, Integer totalParcelas, Titulo tituloOrigem) {
        ...
        if (numeroParcela != null && totalParcelas != null && numeroParcela > totalParcelas) {
            violations.add(new BeanValidationMessage("numeroParcela", "Número da parcela não pode ser maior que o total"));
        }
    ...
    public void definirUnidadeNegocio(UnidadeNegocio unidadeNegocio) {
    ...
    public void atualizar(String descricao, LocalDate dataVencimento) {
        ...
        if (status == StatusTitulo.PAGO || status == StatusTitulo.CANCELADO) {
            violations.add(new BeanValidationMessage("status", "Não é possível alterar título " + status.getDescricao()));
        }
    ...
    public void cancelar() {
        ...
        if (!movimentacoes.isEmpty()) {
            violations.add(new BeanValidationMessage("movimentacoes", "Não é possível cancelar título com movimentações"));
        }
