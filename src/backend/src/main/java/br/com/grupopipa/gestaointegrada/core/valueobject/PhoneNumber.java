...
public class PhoneNumber implements Serializable {
    ...
    public PhoneNumber(String value) {
        ...
        if (value == null || value.isBlank()) {
            violations.add(new BeanValidationMessage("validation.phone.required", "O campo 'Telefone' é obrigatório."));
        ...
        if (numeros.length() < 10 || numeros.length() > 11) {
            violations.add(new BeanValidationMessage("validation.phone.length", "Telefone deve ter 10 ou 11 dígitos."));
            throw new BeanValidationException(violations);
        }
