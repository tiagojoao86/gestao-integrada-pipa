...
/**
 * Filter para definir o tenant ANTES do Spring Security processar a requisição Ordem:
 * HIGHEST_PRECEDENCE + 1 (executa logo após o primeiro filtro do Spring)
 *
 * <p>IMPORTANTE: Este filter executa ANTES da autenticação do Spring Security! Também valida que o
 * tenant do JWT corresponde ao tenant do header
 */
@Slf4j
@Component
@Order(1)
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    try {
      ...
      if (tenantId == null || tenantId.trim().isEmpty()) {
        ...
        httpResponse
            .getWriter()
            .write("{\"error\":\"Tenant não identificado. Header X-Tenant-ID é obrigatório.\"}");
        return;
      }
      ...
      log.info(
          "✅ TENANT DEFINIDO - Header: '{}', Schema: '{}', Path: {} {}",
          tenantId,
          schemaName,
      ...
      if (!requestPath.contains("/authenticate") && !requestPath.contains("/admin/tenants")) {
        ...
        if (authHeader != null && authHeader.startsWith("Bearer ")) {
          ...
          if (tokenTenantId != null && !tokenTenantId.equals(schemaName)) {
            log.warn(
                "❌ TENANT MISMATCH - Token tenant: '{}', Header tenant: '{}'",
                tokenTenantId,
                schemaName);
            ...
            httpResponse
                .getWriter()
                .write("{\"error\":\"Token não pertence a este tenant. Acesso negado.\"}");
            return;
          }
