...
public class Session {
  ...
  /**
   * Retorna os IDs das Unidades de Negócio do usuário a partir do token JWT. Se não houver token ou
   * claim, retorna conjunto vazio (sem filtro - admin).
   *
  ...
  public static Set<UUID> getUnidadeNegocioIds() {
    ...
    if (principal instanceof Jwt jwt) {
      ...
      if (unidades != null && !unidades.isEmpty()) {
        return unidades.stream().map(UUID::fromString).collect(Collectors.toSet());
      }
    }
