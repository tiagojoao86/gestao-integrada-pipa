...
public class PerfilServiceImpl
    ...
    protected PerfilEntity mergeEntityAndDTO(PerfilEntity entity, PerfilDTO dto) {
        if (Objects.isNull(entity)) {
            PerfilEntity newEntity = new PerfilEntity.Builder().nome(dto.getNome()).build();
    ...
    public List<PerfilParaVinculoDTO> listarParaVinculo() {
        return repository.findAll().stream()
                .map(
                        perfil -> PerfilParaVinculoDTO.builder().id(perfil.getId()).nome(perfil.getNome()).build())
                .toList();
    }

    private PerfilModuloEntity buildPerfilModuloEntity(PerfilEntity perfil, PerfilModuloDTO dto) {
        var modulo = moduloRepository
                .findById(dto.getModuloId())
                .orElseThrow(
                        () -> new EntityNotFoundException("Modulo", "id", dto.getModuloId().toString()));
