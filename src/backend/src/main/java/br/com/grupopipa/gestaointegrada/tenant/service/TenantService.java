...
/**
 * Serviço para gerenciar tenants Utiliza Flyway para executar migrations em cada schema de tenant
 */
@Service
@Slf4j
...
public class TenantService {
  ...
  /**
   * Cria um novo tenant 1. Salva no banco (schema public) 2. Cria o schema do tenant (FORA da
   * transação) 3. Executa migrations no schema do tenant
   */
  public Tenant criarTenant(
      String tenantId, String nome, String numeroDocumento, TenantPlano plano) {
  ...
  /**
   * Cria o schema no PostgreSQL Usa conexão em AUTOCOMMIT para garantir que o schema seja criado
   * imediatamente
   */
  private void criarSchema(String schemaName) {
    try {
      ...
      String existingSchema =
          jdbcTemplate
              .query(checkSql, (rs, rowNum) -> rs.getString("schema_name"), schemaName)
              .stream()
              .findFirst()
              .orElse(null);
      ...
      String createdSchema =
          jdbcTemplate
              .query(validationSql, (rs, rowNum) -> rs.getString("schema_name"), schemaName)
              .stream()
              .findFirst()
              .orElse(null);
  ...
  /**
   * Executa migrations do Flyway no schema do tenant Usa as mesmas migrations que estão em
   * db/migration/tenant Elimina duplicação: migrations são mantidas apenas em um lugar!
   */
    ...
    try {
      ...
      Flyway flyway =
          Flyway.configure()
              ...
              .locations(
                  "classpath:db/tenant-migrations") // Migrations específicas de tenant (pasta
              // separada)
      ...
      log.error(
          "Erro ao executar Flyway migrations no schema {}: {}", schemaName, e.getMessage(), e);
