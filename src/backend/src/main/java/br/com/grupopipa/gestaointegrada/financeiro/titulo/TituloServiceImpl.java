...
public class TituloServiceImpl
  ...
  public TituloServiceImpl(
      ...
      br.com.grupopipa.gestaointegrada.financeiro.titulocategoria.TituloCategoriaRepository
          tituloCategoriaRepository) {
  ...
  protected Titulo mergeEntityAndDTO(Titulo entity, TituloDTO dto) {
    ...
    Money valorDesconto =
        dto.getValorDesconto() != null ? Money.positiveOrZero(dto.getValorDesconto()) : null;
    Money valorJuros =
        dto.getValorJuros() != null ? Money.positiveOrZero(dto.getValorJuros()) : null;
    Money valorMulta =
        dto.getValorMulta() != null ? Money.positiveOrZero(dto.getValorMulta()) : null;
  ...
  protected TituloDTO buildDTOFromEntity(Titulo entity) {
    List<TituloSetorDTO> setoresDTO =
        entity.getSetores().stream()
            .map(
                ts ->
                    TituloSetorDTO.builder()
  ...
  /**
   * Override do método list para usar projeção otimizada com cálculo de valorPago Evita N+1 queries
  ...
  public PageDTO<TituloGridDTO> list(FilterDTO filter, Pageable pageable) {
    ...
    org.springframework.data.jpa.domain.Specification<Titulo> specification =
        this.buildSpecification(filter);
    ...
    List<TituloGridDTO> gridDTOs =
        page.getContent().stream().map(this::buildGridDTOFromProjection).toList();
  ...
  public List<TituloDTO> searchByQuery(String q, int size) {
    ...
    Specification<Titulo> descricaoSpec =
        (root, query, cb) -> cb.like(cb.lower(root.get("descricao")), pattern);

    Specification<Titulo> numeroDocSpec =
        (root, query, cb) -> cb.like(cb.lower(root.get("numeroDocumento")), pattern);
    ...
    var page =
        ((JpaSpecificationExecutor<Titulo>) this.repository)
