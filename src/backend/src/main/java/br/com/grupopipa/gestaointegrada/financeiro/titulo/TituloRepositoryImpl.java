...
/**
 * Implementação customizada para consultas otimizadas de Titulo Usa CriteriaQuery para combinar
...
public class TituloRepositoryImpl implements TituloRepositoryCustom {
  ...
  public Page<TituloProjection> findAllProjected(Specification<Titulo> spec, Pageable pageable) {
    ...
    Expression<BigDecimal> valorPagoExpr =
        cb.coalesce(
            cb.sum(
  ...
  /**
   * Implementação concreta de TituloProjection Classe interna para encapsular os dados retornados
   * pela projeção
   */
