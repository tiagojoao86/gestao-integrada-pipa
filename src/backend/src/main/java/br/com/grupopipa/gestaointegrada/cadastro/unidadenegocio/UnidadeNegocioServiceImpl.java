...
public class UnidadeNegocioServiceImpl
    extends CrudServiceImpl<
        UnidadeNegocioDTO, UnidadeNegocioGridDTO, UnidadeNegocio, UnidadeNegocioRepository>
    implements UnidadeNegocioService {

  public UnidadeNegocioServiceImpl(
      UnidadeNegocioRepository repository, Specifications<UnidadeNegocio> specifications) {
  ...
  public List<UnidadeNegocioDTO> listarAtivas() {
    return repository.findByAtivaTrue().stream().map(this::buildDTOFromEntity).toList();
  }

  @Override
  public List<UnidadeNegocioDTO> listarDisponiveisParaUsuario() {
    Set<UUID> unidadesPermitidas =
        br.com.grupopipa.gestaointegrada.core.Session.getUnidadeNegocioIds();

    return repository.findByAtivaTrue().stream()
        .filter(
            unidade -> unidadesPermitidas.isEmpty() || unidadesPermitidas.contains(unidade.getId()))
  ...
  public List<UnidadeNegocioDTO> listarTodasDisponiveis() {
    return repository.findByAtivaTrue().stream().map(this::buildSimpleDTO).toList();
  }
