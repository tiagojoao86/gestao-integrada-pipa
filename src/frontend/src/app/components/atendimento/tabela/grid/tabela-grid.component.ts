...
@Component({
  ...
  styleUrl: './tabela-grid.component.css',
})
export class TabelaGridComponent {
  titulo: string = $localize`Tabelas de Preços`;
  ...
  columns: ColumnModel<TabelaGridDTO>[] = [
    {
      name: 'nome',
      label: $localize`Nome`,
      getValue: (e: TabelaGridDTO) => e.nome ?? '',
    },
    {
      name: 'tipo',
      label: $localize`Tipo`,
    ...
    },
    {
      ...
      getValue: (e: TabelaGridDTO) => e.ativo ? $localize`Sim` : $localize`Não`,
    },
  ];

  tableActions: ActionModel<TabelaGridDTO>[] = [];
  toolbarActions: ToolbarActionModel[] = [];

  filtros: FilterProperty[] = [
    { property: 'nome', label: $localize`Nome`, filterType: FilterType.TEXT },
    { property: 'tipo', label: $localize`Tipo`, filterType: FilterType.TEXT },
  ];
  ...
  private service: TabelaService = inject(TabelaService);
  private auth: AuthService = inject(AuthService);
  private dialogService: DialogService = inject(DialogService);

  constructor() {
    const canView = this.auth.hasAuthorityVisualizarToModulo(SystemModuleKey.ATENDIMENTO_TABELA);
    const canDelete = this.auth.hasAuthorityDeletarToModulo(SystemModuleKey.ATENDIMENTO_TABELA);
    const canEdit = this.auth.hasAuthorityEditarToModulo(SystemModuleKey.ATENDIMENTO_TABELA);
    const canAudit = this.auth.hasAuthorityAuditarToModulo(SystemModuleKey.ATENDIMENTO_TABELA);
