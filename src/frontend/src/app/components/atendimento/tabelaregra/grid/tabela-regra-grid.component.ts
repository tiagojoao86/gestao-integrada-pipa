...
@Component({
  ...
  providers: [TabelaRegraService],
  templateUrl: './tabela-regra-grid.component.html',
})
export class TabelaRegraGridComponent {
  titulo: string = $localize`Regras de Tabela`;
  ...
  rows: TabelaRegraGridDTO[] = [];

  columns: ColumnModel<TabelaRegraGridDTO>[] = [
    {
      name: 'convenioNome',
      label: $localize`Convênio`,
      getValue: (e: TabelaRegraGridDTO) => e.convenioNome ?? '',
    },
    {
      name: 'convenioCategoriaNome',
      label: $localize`Categoria`,
      getValue: (e: TabelaRegraGridDTO) => e.convenioCategoriaNome ?? $localize`(geral)`,
    },
    {
      name: 'tabelaNome',
      label: $localize`Tabela de Preços`,
      getValue: (e: TabelaRegraGridDTO) => e.tabelaNome ?? '',
    },
  ];

  tableActions: ActionModel<TabelaRegraGridDTO>[] = [];
  toolbarActions: ToolbarActionModel[] = [];

  filtros: FilterProperty[] = [
    { property: 'convenioNome', label: $localize`Convênio`, filterType: FilterType.TEXT },
    { property: 'tabelaNome', label: $localize`Tabela`, filterType: FilterType.TEXT },
  ];
  ...
  private service: TabelaRegraService = inject(TabelaRegraService);
  private auth: AuthService = inject(AuthService);
  private dialogService: DialogService = inject(DialogService);

  constructor() {
    const canView = this.auth.hasAuthorityVisualizarToModulo(SystemModuleKey.ATENDIMENTO_TABELA_REGRA);
    const canDelete = this.auth.hasAuthorityDeletarToModulo(SystemModuleKey.ATENDIMENTO_TABELA_REGRA);
    const canEdit = this.auth.hasAuthorityEditarToModulo(SystemModuleKey.ATENDIMENTO_TABELA_REGRA);
    const canAudit = this.auth.hasAuthorityAuditarToModulo(SystemModuleKey.ATENDIMENTO_TABELA_REGRA);

    if (canView) {
      this.tableActions.push({
        icon: 'edit_note',
        title: $localize`Editar`,
        action: (e: TabelaRegraGridDTO) => this.openDetail.emit(e.id!),
      });
    }
    ...
    if (canAudit) {
      this.tableActions.push({
        ...
        action: (e: TabelaRegraGridDTO) => this.loadAuditInfo(e.id!),
      });
    }
